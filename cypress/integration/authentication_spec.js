...
describe('Authentication', function () {
    ...
    it('Goes to register page with authorize redirectTo param and no client or final callback url', function () {
        let redirectUrl = apiUrl + "/api/oauth2/authorize";
        let encodedRedirectTo = encodeURIComponent(redirectUrl);
        cy.visitApi('/api/v2/auth/register?redirectTo=' + encodedRedirectTo)
        cy.enterNewUserCredentials(false);
        cy.log("It should skip authorize page because we don't have a client id")
    ...
    it('Logs in and out with username and password', function () {
        if (!Cypress.env('API_HOST')) {
            cy.log(`Cypress.env('API_HOST') is empty so falling back to ${API_HOST}`)
        }
